...
const getCartItems = async (session: any) => {
  ...
  const data = await User.findOne({ email: session?.user?.email }).populate("cart") as User;
  return data?.cart || [];
};
const Cart = async () => {
  ...
  return (
    <div className="lg:flex justify-center mt-5  gap-7">
      <div className="bg-gray-50 px-7 rounded-md py-7 mb-4">
        ...
        {cartItems &&
          cartItems.map((item: CartItem) => <CartItem key={item._id.toString()} _id={item._id.toString()} price={item.price} productName={item.productName} description={item.description} images={item.images} />)}
      </div>
      <Checkout />
    </div>
  );
